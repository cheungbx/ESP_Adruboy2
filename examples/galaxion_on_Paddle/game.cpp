...
{
  mArduboy.pollButtons();
  switch (mState) {
  case GAME_ST_TITLE:
    if (mArduboy.justReleased(LEFT_BUTTON)) {
      switch (mCursor) {
      case 0:
  ...
  break;
    ...
    } else if (mArduboy.justReleased(RIGHT_BUTTON)) {
      switch (mCursor) {
      case 0:
	mMode = GAME_MODE_NORMAL;
	break;
  ...
  mPaddle = true;
    ...
    } else if (mCursor > 0 && mArduboy.justReleased(UP_BUTTON)) {
      mCursor --;
    } else if (mCursor < 2 && mArduboy.justReleased(DOWN_BUTTON)) {
      mCursor ++;
    }
  case GAME_ST_GAMEOVER:
    if (mArduboy.justReleased(A_BUTTON) || mArduboy.justReleased(B_BUTTON)) {
...
//  mArduboy.print("PUSH A BUTTON");
  ...
  mArduboy.setCursor(x + 8 + 3, y);
  if (mPaddle) 
    mArduboy.print(" PADDLE ");
  else
    mArduboy.print(" BUTTON ");

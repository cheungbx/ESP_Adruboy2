...
void loop() {
 ...
 // we clear our screen to black
  ...
  if(arduboy.pressed(B_BUTTON)) { arduboy.print("B"); beep.tone(beep.freq(880.251), 5);} else arduboy.print ("O");
   ...
   arduboy.print (arduboy.paddle());
   x = constrain (map (arduboy.paddle(), minPaddle, maxPaddle, 0, X_MAX), 0, X_MAX);

   
  
   arduboy.setCursor(64, 10);
   arduboy.print("P2=");
   arduboy.setCursor(84, 10);
   arduboy.print (arduboy.paddle2());
   y = constrain (map (arduboy.paddle2(), minPaddle, maxPaddle, 0, Y_MAX), 0, Y_MAX);
